...
@router.get("/cost-structure", response_model=CostStructureOut)
def cost_structure(
...
@router.get("/clinical-top", response_model=ClinicalTopOut)
def clinical_top(
    db: Session = Depends(get_db),
    limit: int = 10,
    dept_name: str | None = None,
...
@router.get("/disease-priority", response_model=DiseasePriorityOut)
